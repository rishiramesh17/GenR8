...
import { ArrowLeft, Home, Save, FolderPlus } from 'lucide-react';
import { Button } from "@/components/ui/button";
import CreditBadge from '@/components/ui/CreditBadge';
import ArchitectureForm from '@/components/generators/ArchitectureForm';
...
export default function ArchitectureGenerator() {
  ...
  const buildPrompt = (formData) => {
    const roomType = formData.roomType.replace(/_/g, ' ');
    const style = formData.designStyle.replace(/_/g, ' ');
    const size = formData.roomSize.replace(/_/g, ' ');
    const colorPalette = formData.colorPalette.replace(/_/g, ' ');
    const lighting = formData.lighting.replace(/_/g, ' ');
    const windowConfig = formData.windowConfig.replace(/_/g, ' ');
    const viewAngle = formData.viewAngle.replace(/_/g, ' ');
    
    let prompt = `Professional architectural visualization of a ${size} ${roomType} in ${style} style. ${viewAngle} perspective.`;
    
    if (formData.furniturePlacement && formData.furniturePlacement.length > 0) {
      const furniture = formData.furniturePlacement.map(f => f.replace(/_/g, ' ')).join(', ');
      prompt += ` Furniture layout includes: ${furniture}.`;
    }
    
    prompt += ` ${windowConfig}.`;
    prompt += ` ${lighting} lighting scheme.`;
    prompt += ` ${colorPalette} color palette.`;
    
    if (formData.specialFeatures && formData.specialFeatures.length > 0) {
      const features = formData.specialFeatures.map(f => f.replace(/_/g, ' ')).join(', ');
      prompt += ` Special features: ${features}.`;
    }
    ...
    prompt += ' Photorealistic rendering, high-resolution, professional interior design, clean composition, architecturally accurate.';
  ...
  const handleVariation = async (image) => {
    ...
    try {
      const result = await base44.integrations.Core.GenerateImage({
        prompt: image.prompt + ' Variation with slight changes in layout or perspective.',
      });
    ...
    <div className="min-h-screen bg-slate-950">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-semibold text-white">Room Designer</h1>
                  <p className="text-xs text-slate-500">Visualize & design your ideal space</p>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <h2 className="text-lg font-semibold text-white mb-6">Room Configuration</h2>
            <ArchitectureForm onGenerate={handleGenerate} isGenerating={isGenerating} />
          </motion.div>
