...
import LogoForm from '@/components/generators/LogoForm';
...
export default function LogoGenerator() {
  ...
  const buildPrompt = (formData) => {
    ...
    const industry = formData.industry.replace(/_/g, ' ');
    const complexity = formData.complexity[0] < 33 ? 'simple and minimalist' : 
                       formData.complexity[0] < 66 ? 'balanced' : 'detailed and intricate';
    
    let prompt = `Professional logo design for "${formData.brandName}"`;
    if (formData.tagline) prompt += ` with tagline "${formData.tagline}"`;
    prompt += `. ${style} style, ${industry} industry. ${complexity} design.`;
    
    if (formData.colorPreset) {
      prompt += ` Color palette: ${formData.colorPreset.name} (${formData.colorPreset.colors.join(', ')}).`;
    }
    ...
    prompt += ' High quality, vector-style, clean edges, professional branding, white background.';
  ...
  const handleVariation = async (image) => {
    ...
    try {
      const result = await base44.integrations.Core.GenerateImage({
        prompt: image.prompt + ' Variation with slight changes.',
      });
    ...
    <div className="min-h-screen bg-slate-950">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-semibold text-white">Logo Generator</h1>
                  <p className="text-xs text-slate-500">Create professional brand logos</p>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <h2 className="text-lg font-semibold text-white mb-6">Design Parameters</h2>
            <LogoForm onGenerate={handleGenerate} isGenerating={isGenerating} />
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6"
          >
            <h2 className="text-lg font-semibold text-white mb-6">Generated Results</h2>
            <GenerationGallery
