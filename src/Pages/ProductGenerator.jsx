...
import { ArrowLeft, Package, Save, FolderPlus } from 'lucide-react';
import { Button } from "@/components/ui/button";
import CreditBadge from '@/components/ui/CreditBadge';
import ProductForm from '@/components/generators/ProductForm';
...
export default function ProductGenerator() {
  ...
  const buildPrompt = (formData) => {
    const category = formData.productCategory.replace(/_/g, ' ');
    const style = formData.visualStyle.replace(/_/g, ' ');
    ...
    let prompt = `${style} product visualization of ${formData.productDescription}`;
    
    // Add category context
    prompt += ` Product category: ${category}.`;
    
    // Add view angle
    prompt += ` ${viewAngle} perspective.`;
    
    // Add materials
    if (formData.materials.length > 0) {
      const materials = formData.materials.map(m => m.replace(/_/g, ' ')).join(', ');
    ...
    // Add background
    prompt += ` ${background} background setting.`;
    
    // Add detail level
    const detailLevel = formData.detailLevel[0] || 3;
    const detailDescriptor = detailLevel === 1 ? 'clean and simple' : 
                           detailLevel === 2 ? 'balanced detail' : 
                           'highly detailed and intricate';
    prompt += ` ${detailDescriptor} rendering.`;
    
    // Add annotations if requested
    if (formData.includeDimensions) {
      prompt += ' Include visible dimension labels and measurements.';
    }
    
    if (formData.includeAnnotations) {
      prompt += ' Include feature callouts and component labels.';
    }
    
    // Add additional features
    if (formData.additionalFeatures) {
      prompt += ` ${formData.additionalFeatures}.`;
    }
    
    // Add quality descriptors
    prompt += ' Professional product photography quality, commercial-ready, high resolution, studio lighting, sharp focus.';
  ...
  const handleVariation = async (image) => {
    ...
    try {
      const result = await base44.integrations.Core.GenerateImage({
        prompt: image.prompt + ' Variation with slight changes in design or perspective.',
      });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-semibold text-white">Product Designer</h1>
                  <p className="text-xs text-slate-500">Transform ideas into product visuals</p>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <ProductForm onGenerate={handleGenerate} isGenerating={isGenerating} />
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6"
          >
            <h2 className="text-lg font-semibold text-white mb-6">Generated Products</h2>
            <GenerationGallery
